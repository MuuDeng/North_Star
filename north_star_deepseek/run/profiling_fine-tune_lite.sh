...
time /usr/mpi/gcc/openmpi-4.1.7a1/bin/mpirun \
  ...
  bash -lc '
    ...
    # === เลือกโหมด ===
    PROFILE_MODE="${PROFILE_MODE:-full}"  # "lite" | "full"

    if [[ "$PROFILE_MODE" == "full" ]]; then
      nsys profile -t cuda,nvtx,osrt,mpi,ucx \
        --mpi-impl=openmpi \
        --gpu-metrics-device=all --gpu-metrics-frequency=200 \
      ...
      nsys profile -t cuda,nvtx,osrt \
        --sample=cpu --cpuctxsw=none --backtrace=none \
        --force-overwrite=true \
        -o ${outdir}/sglang_${PBS_JOBID}_rank${OMPI_COMM_WORLD_RANK} \
        ${HOME}/scratch/tanathep/py312/bin/python3 -m sglang.bench_offline_throughput \
          --model-path ${HOME}/scratch/model/DeepSeek-R1 \
