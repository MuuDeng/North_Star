...
-tag-output -output-filename ${HOME}/scratch/nattanon/run/sglang.${PBS_JOBID} \
-x PATH \
-x NCCL_DEBUG=INFO \
-x DIST_INIT_ADDR=$(head -n 1 $PBS_NODEFILE) \
bash -c 'time nsys profile \
--output=${HOME}/scratch/nattanon/profiling/nsys_reports/sglang_profile_by-tonmai.${PBS_JOBID}.${OMPI_COMM_WORLD_RANK} \
--trace=cuda,nvtx,mpi,osrt \
${HOME}/scratch/tanathep/py312/bin/python3 \
